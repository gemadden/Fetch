...
powerup = False
power_ups_1 = [
    pygame.Rect(random.randint(0, 700), 510, 50, 50),
]

power_ups_2= [
    pygame.Rect(random.randint(1000, 1500), 735, 50, 50),
]
...
while is_game_running:
    ...
    if game_mode == "instructions":
        ...
        text_surface = name_font.render("Goal: Catch 35 tennis balls before 3 reach the ground!", True, [0, 0, 0,])
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if level_one_rect.collidepoint(mouse_pos):
                    game_mode = "level_one"
                    if use_audio:
                        pygame.mixer.music.load("day_music.MP3")
                ...
                if level_two_rect.collidepoint(mouse_pos) and level_one_complete:
                    ...
                    if use_audio:
                        ...
                        pygame.mixer.music.load("sunset_music.WAV")
                        pygame.mixer.music.play(loops = -1)
                ...
                if level_three_rect.collidepoint(mouse_pos) and level_two_complete:
                    ...
                    if use_audio:
                        pygame.mixer.music.stop()
                        pygame.mixer.music.load("night_music.MP3")
                        pygame.mixer.music.play(loops = -1)
    ...
    if game_mode == "level_one":
        ...
        if not paused_mode_1:
            ...
            new_powerups_1 = []
            for powerup_1 in power_ups_1: # loop through all objects
                collide = powerup_1.colliderect(player_rect) # for object in the index, did it get hit?
                if collide:
                    powerup = True
                    new_powerup_1 = pygame.Rect(random.randint(0, 700), 510, 50, 50),
                    new_powerup_1.bottom = 0
                    new_powerups_1.append(new_powerup_1)
                    continue
                new_powerups_1.append(powerup_1)
            powerups_1 = new_powerups_1
